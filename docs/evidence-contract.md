...
HTTP routes are `POST /api/v1/analyses`, `GET` and `DELETE /api/v1/analyses/{analysis_id}`, and `GET /api/v1/health`. The generated contract is committed as [openapi.json](openapi.json); CI fails on drift.
