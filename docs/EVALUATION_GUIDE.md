...
Before launching a new training or evaluation cycle, run the sanity checks below to keep datasets, adapters, and the router aligned:

```bash
# 1. Validate training corpora (checks for illegal moves, FEN issues, placeholders)
python3 scripts/test_data_quality.py

# 2. Inspect dataset profiles generated by the training script
ls reports/dataset_profiles/
cat reports/dataset_profiles/uci_training_mix.json

# 3. Review live router telemetry (cache health, expert accuracy trends)
curl http://localhost:5000/api/router/diagnostics | jq
...
### Router Retraining Quickstart

Leverage the new decision log to refresh the router on recent production traffic:

```bash
python3 scripts/train_moe_router.py \
  --epochs 40 \
  --batch-size 64 \
  --learning-rate 0.0018 \
  --max-per-expert 450 \
  --decision-log reports/moe/routing_decisions.jsonl \
  --min-log-confidence 0.8 \
  --max-log-samples 300
```

The script auto-balances curated evaluation suites with confident real-world routes. Adjust `--min-log-confidence` upward if you spot noise in `routing_decisions.jsonl`.
```
