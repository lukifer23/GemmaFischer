...
models reject extra fields, and the HTTP boundary rejects bodies above 300 KiB
...
Imported PGNs, hidden study answers, practice attempts, review cards, completed
analyses, server-owned sessions, and redacted tutor state are retained
...
profile resolves one pinned revision with local-files-only mode. If an adapter
is configured, it must be an explicit local directory with one safetensors file,
an adapter config, and an optional exact SHA-256 pin. Lifecycle
