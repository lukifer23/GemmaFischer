#!/usr/bin/env python3
"""
Complete Director Expert Training Script

Advanced training script for completing Director expert training to 1000+ steps
with enhanced data quality validation, progress monitoring, and stability improvements.

Uses instruction tuning (--use_instruction_collator) for proper strategic analysis.
"""
...
def run_director_training(max_steps=1000, timeout_minutes=180, force_restart=False):
    """Run Director expert training with enhanced stability and monitoring."""

    print("=" * 60)
    print("🏆 Complete Director Expert Training")
    print("=" * 60)
    ...
    try:
        ...
        print("\n" + "=" * 60)
        if rc == 0:
            print("✅ Director Training Completed Successfully!")
            ...
            print(f"❌ Director Training Failed (Exit Code: {rc})")
        ...
        with open(data_path, 'r', encoding='utf-8') as f:
            for line_num, line in enumerate(f, 1):
                try:
                    ...
                    # For director expert, response contains strategic analysis but should have a valid UCI move
                    # Look for UCI move pattern in the response
                    import re
                    uci_pattern = r'\b([a-h][1-8][a-h][1-8][qrbn]?\b)'
                    uci_matches = re.findall(uci_pattern, response.lower())

                    valid_move_found = False
                    for potential_move in uci_matches:
                        try:
                            move_obj = chess.Move.from_uci(potential_move)
                    ...
                    if not valid_move_found:
                        if line_num <= 5:  # Show first few errors
                            print(f"   Sample {line_num} error: no valid UCI move found in response: {repr(response[:100])}")
                        invalid_samples += 1
                        continue
...
def main():
    ...
    print("🎯 ChessGemma Director Expert Training Completion Script")
    print("=" * 50)

    # Validate data quality
    if not args.skip_validation:
        if not validate_director_data():
    ...
    try:
        response = input("\n🚀 Start Director training? (y/N): ").strip().lower()
    ...
    # Run training
    success = run_director_training(args.max_steps, args.timeout_minutes, args.force_restart)

    if success:
        print("\n🎉 Director Expert training completed successfully!")
        print("🔍 You can now evaluate the model performance and proceed with other experts.")
    else:
        print("\n❌ Director Expert training failed.")
